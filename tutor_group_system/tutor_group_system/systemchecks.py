"""
The :py:mod:`tutor_group_system` application ships with some custom system
checks which ensure that required settings are present. These system checks are registered by
the :py:class:`~tutor_group_system.apps.Config` class's
:py:meth:`~tutor_group_system.apps.AssetsConfig.ready` method.
    ...
    for idx, name in enumerate(REQUIRED_SETTINGS):
        ...
        if value is None or value == '':
            errors.append(Error(
                ...
                id='tutor_group_system.E{:03d}'.format(idx + 1),
